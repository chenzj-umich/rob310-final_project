...
def smoothing(prev, unsmoothed, alpha):
    updated = alpha * unsmoothed + (1 - alpha) * prev
    return updated
...
if __name__ == "__main__":
    ...
    try:
        ...
        #while True:
            ...
            encoder_read = enc.read() * LEFT_ENC_POLARITY
            speed = calc.calculateSpeed(encoder_read, current_time)
            
            smoothed_prev = smoothed
            smoothed = smoothing(smoothed_prev, setpoint, ALPHA)
            
            pid_input = pid.update(smoothed - speed, dt)
            if pid_input > 1:
                pid_input = 0.99
            elif pid_input < -1:
                pid_input = -0.99
            ...
            motor.set(pid_input * LEFT_MOTOR_POLARITY)
            m0_csns = machine.ADC(26)
            data = m0_csns.read_u16()
            current = data/65535 * 3.1
            print(f'{motor.read()}, {speed}, {setpoint}, {current}, {dt}')
        motor.set(0)
